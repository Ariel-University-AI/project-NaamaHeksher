...
def _estimate_points_raw(area_sqm: float, map_type: str = "", terrain: str = "mixed") -> int:
    """Rule-based survey-points estimate: initial density 0.4 pts/sqm decays exponentially; total always grows with area."""
    import math
    D0      = 200.0 / 500.0  # initial density: 0.4 pts/sqm
    DECAY_K = 5e-5           # density halves roughly every 13,900 sqm; saturation ~8,000 pts (before factors)
    base    = (D0 / DECAY_K) * (1.0 - math.exp(-DECAY_K * area_sqm))
    map_factors    = {"RZ": 1.0, "TZR": 0.5}
    terrain_factors = {"open": 0.25, "mixed": 0.75, "built": 1.0}
...
def _calc_pricing(area_sqm: float, survey_points: float, distance_km: float) -> dict:
    """Calculate pricing breakdown: recommended quote, actual hours, cost estimate."""
    import math as _m

    dunams = area_sqm / 1000.0

    # ── Price by area ────────────────────────────────────────────────────────
    ceil_d = _m.ceil(dunams)
    if area_sqm <= 1000:
        price_area = 3500.0
    elif area_sqm <= 5000:
        price_area = 3500.0 + ceil_d * 700.0
    else:
        price_area = 7000.0 + (ceil_d - 5) * 1000.0

    # ── Hours & price by points (tiered @ 850₪/hr) ──────────────────────────
    TIERS = [(300, 150), (435, 135), (555, 120), (660, 105), (float("inf"), 90)]
    pts_remaining = survey_points
    prev_ceiling  = 0
    hours_pts     = 0.0
    for ceiling, rate in TIERS:
        tier_size = ceiling - prev_ceiling
        taken     = min(pts_remaining, tier_size)
        hours_pts += taken / rate
        pts_remaining -= taken
        prev_ceiling = ceiling
        if pts_remaining <= 0:
            break
    price_pts = hours_pts * 850.0

    # ── Distance factor ──────────────────────────────────────────────────────
    dist_factor = 1.0 + max(0.0, distance_km - 20.0) * 0.0025

    # ── Recommended quoted price ─────────────────────────────────────────────
    recommended_price = max(price_area, price_pts) * dist_factor

    # ── Actual hours (max of points-hours, area-hours, 4.0 minimum) ──────────
    if dunams <= 5:
        hrs_per_dunam = 1.5
    elif dunams <= 50:
        hrs_per_dunam = 1.5 - (dunams - 5) / (50 - 5) * (1.5 - 1.2)
    else:
        hrs_per_dunam = 1.2
    hours_area   = dunams * hrs_per_dunam
    actual_hours = max(hours_pts, hours_area, 4.0)

    full_blocks  = int(actual_hours // 8)
    remaining    = actual_hours % 8
    actual_cost  = full_blocks * 5500.0 + remaining * 690.0

    return {
        "price_by_area":      round(price_area, 0),
        "price_by_points":    round(price_pts, 0),
        "hours_by_points":    round(hours_pts, 2),
        "hours_by_area":      round(hours_area, 2),
...
def _profit_predict(params: dict) -> dict:
    ...
    pricing         = _calc_pricing(area, pts, dist)
    actual_cost_est = pricing["actual_cost"]
    ...
    return {
        ...
        "pricing_breakdown": {
            "price_by_area":     pricing["price_by_area"],
            "price_by_points":   pricing["price_by_points"],
            "recommended_price": pricing["recommended_price"],
            "actual_hours":      pricing["actual_hours"],
            "hours_by_points":   pricing["hours_by_points"],
            "hours_by_area":     pricing["hours_by_area"],
            "dist_factor":       pricing["dist_factor"],
        },
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif parsed.path == "/api/training-replace":
            rows = params.get("rows", [])
            try:
                TRAINING_DB.write_text(
                    json.dumps(rows, ensure_ascii=False, indent=2), "utf-8"
                )
                self._send_json(200, {"ok": True, "total": len(rows)})
            except Exception as e:
                self._send_json(500, {"error": str(e)})
